...
data = np.load('../NR-ER/NR-ER-test/names_onehots.npy', allow_pickle = True)
...
labels = np.genfromtxt('../NR-ER/NR-ER-test/names_labels.csv',delimiter=',',dtype=None)
label = []
...
with tf.Session as sess:
    ...
    for i in range(datalength):
        if i % 10 == 0:
            print(compute_accuracy(onehotArr[i],label[i],size))
